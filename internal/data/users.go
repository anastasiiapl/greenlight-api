...
// the plainText is a pointer to a string
// so we can distinguish between password being nil and
// empty string ""
...
func (m *UsersModel) GetForToken(tokenScope string, plainTextToken string) (*User, error) {
	// Calculate the SHA-256 hash of the plaintext token provided by the client.
	// Remember that this returns a byte *array* with length 32, not a slice.
	hashToken := sha256.Sum256([]byte(plainTextToken))
	...
	args := []any{hashToken[:], tokenScope, time.Now()}
	var user User
...
// func ValidatePasswordHash(passwordText string, expectedHash string) bool {
// 	hash := sha256.Sum256([]byte(passwordText))

// 	return string(hash[:]) == expectedHash
// }
